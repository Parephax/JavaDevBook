...
public class SpriteTest2 {
    ...
    public void drawFade(Graphics2D g, long elapsedTime) {
        long time;
        if (elapsedTime <= FADE_TIME) {
            time = FADE_TIME - elapsedTime;
        } else if (elapsedTime > DEMO_TIME - FADE_TIME) {
            time = FADE_TIME - DEMO_TIME + elapsedTime;
        } else {
            return;
        }
